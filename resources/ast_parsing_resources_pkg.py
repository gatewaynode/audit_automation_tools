...
####### The bulk of the code of the following method stems from the distance package. All kudos for the implementation go to the package maintener.
def levenshtein_free_deletion(seq1, seq2, max_dist=-1, normalized=False):
    ...
    for x in range(1, len1 + 1):
        ...
        for y in range(1, len2 + 1):
            ...
            insertion_cost = column[y] + 1
            deletion_cost = column[y - 1] + 1
            substitution_cost = last + cost
            column[y] = min(insertion_cost, deletion_cost, substitution_cost)
            if deletion_cost == column[y]:  # i.e. if the deletion was favored,
                column[
                    y
                ] -= (
                    1
                )  # remove the cost of the deletion (because in our context it is free).
            last = old
