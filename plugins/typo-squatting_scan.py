...
class Typo_Squatting_Protection(IPlugin):
    def scan(
        ...
        if verbose and not output_json:
            print("Looking for potential typo-squatting in the package")
        ...
        with open(
            f"{output_dir}/typo_squatting_results", "w", encoding="utf-8"
        ) as scan_results:
                ...
                try:
                    ...
                    for top_pkg in top5000_list:
                        if _name_distance_indicator(pkg_name, top_pkg) < threshold:
                            suspicious_list.append(top_pkg)
                            suscpicion_count += 1
                    scan_results.write(
                        f"{pkg_name}- Potential typo squattings detected -{suscpicion_count}- List of potential typo squatted packages -{suscpicion_list}\n"
    ...
    def _loading_top_5000():
        if os.path.exists(f"{plugin_location}/top5000_list.json"):
            ...
            return loading_top_5000()  # Recursive call because I am lazy.

    def _name_distance_indicator(pkg_name_1, pkg_name_2):
        if pkg_name1 == pkg_name_2:
            return
        return (
            2
