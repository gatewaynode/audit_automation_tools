...
sys.path.insert(0,f"{os.path.dirname(sys.argv[0])}/ressources")   # Adds the folder with ressources for patternMatching to the path.
...
def _look_for_malicious_domains(
    ...
    script_location = os.path.dirname(sys.argv[0])  # Required to access scripts and files located in ressources
    ...
    for target in scan_list:
        try:
            ...
            for (dirpath, dirnames, filenames) in os.walk(os.path.join(output_dir,target)):
            ...
            blacklist = None
            if not os.path.exists(f"{script_location}/ressources/precompiled_blacklist_automaton"):
                blacklist = []
                with open(f"{script_location}/ressources/unifiedBlacklist.txt","r",encoding="utf-8") as unified_blacklist:
                    line = unified_blacklist.readline()
                    while line: # We read all the lines until we reach the end of file.
                        blacklist.append(line.rstrip()) #rstrip removes trailling whitespaces and newline characters from strings (those would later on be a problem for the detection)
                        line = unified_blacklist.readline()
            string_matching_output = stringMatching(blacklist,all_files_in_package,precompiledPath=f"{script_location}/ressources/precompiled_blacklist_automaton")
        except Exception as e:
            logging.error(traceback.format_exc())
            scan_errors += 1
        ...
        try:
            ...
            for (text_file_path, start_index, domain) in string_matching_output:
                file.write(f"In file {text_file_path}, at index {start_index} found the malicious domain {domain};\n")
            file.close()
        except Exception as e:
            logging.error(traceback.format_exc())
            scan_errors += 1
...
def _try_setting_malicious_domain_lookup(
    verbose=False, debug=False, output_json=False
):
    "Needed to make sure that the installation is ready to search for blacklisted domains in tested packages"

    script_location = os.path.dirname(sys.argv[0])   # Needed to access the resources folder folder, which contains among other things the lacklist for malicious domains and the pattern matching script

    is_pattern_matching_correctly_setup = True # Assuming that everything will work unless an ovious problem show up.
    ...
    if not os.path.exists(f"{script_location}/ressources/unifiedBlacklist.txt"):
        if verbose and not output_json:
            print("The malicious domains blacklist was not found, the script will try to create it from the local archive")
        try:
            zipped_blacklist = zipfile.ZipFile(f"{script_location}/ressources/unifiedBlacklist.zip", "r")
        except FileNotFoundError:
            is_pattern_matching_correctly_setup = False
            raise RuntimeWarning("You lack a file called unifiedBlacklist.zip in the folder ressources.\nIt countains a list of malicious URL to look for in scanned packages.\nOther functionalities are still available.")
        if is_pattern_matching_correctly_setup: # The script will not try to decompress the archive if said archive is nowhere to be found in the first place.
            try:
                ...
                is_pattern_matching_correctly_setup = False
                raise RuntimeWarning("Could not decompress the archived blacklist.\nIt may have gotten corrupted, or something unpredicted might have happened.\nOther functionalities are still available.")
    if is_pattern_matching_correctly_setup:
        is_pattern_matching_correctly_setup = os.path.exists(f"{script_location}/ressources/patternMatching.py")   # i.e. the setup is correct only if no previous steps have failed and the library exists.
...
def main(package_name, output_dir, verbose, debug, output_json, input_list):
    ...
    is_pattern_matching_correctly_setup = _try_setting_malicious_domain_lookup(verbose, debug, output_json);
            ...
            if parsed_raw_dir_list:
                ...
                if scan_list and package_meta:
                    ...
                    if is_pattern_matching_correctly_setup:
                        ...
                        print("Malicious domains lookup was skipped because it couldn't be correctly setup in the first place.")
        else:
            if verbose and not output_json:
